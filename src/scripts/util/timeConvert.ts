...
const timeAgo = (time, outside = false) => {
    ...
    if (outside) {
        if (asMonths > 0) return `${asMonths} month${asMonths > 1 ? "s" : ""} ago`;
        if (asDays > 0) return `${asDays} day${asDays > 1 ? "s" : ""} ago`;
        if (asHours > 0) return `${asHours} hour${asHours > 1 ? "s" : ""} ago`;
        if (asMinutes > 0) return `${asMinutes} minute${asMinutes > 1 ? "s" : ""} ago`;
        if (asSeconds > 0) return `${asSeconds} second${asSeconds > 1 ? "s" : ""} ago`;
    }
...
export { ensureMillisecondTimestamp, parseTimestamp, timerFrom, dateDiffInDays, timeFull, time, addZero, timeAgo, chartDate, transactionDate, calculateValidatorsEpoch };
