...
const News: React.FC<IComponent> = ({ className }) => {
    const { newsData, newsCategoriesData } = useSelector(
        (state: IRootState) => state.news
    );

    const classes = classNames([styles.news, className]);

    const handleCategories =
        newsCategoriesData?.map((x) => {
            return {
                id: x.id,
        ...
        <div className={classes}>
            <Grid columns={3}>
                <GridWrapper gridWidth={2}>
                    <Tabs
                        tabsName={"newsTabs"}
                        dataArray={handleCategories}
                        defaultEntry={newsData}
                        itemsCount={false}>
                        <NewsList />
                    </Tabs>
                </GridWrapper>
