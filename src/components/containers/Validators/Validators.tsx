...
const Validators: React.FC<IComponent> = ({ className }) => {
    const { validatorsLocations, validators } = useSelector(
    ...
    const [lastReconfigurationTime, setLastReconfigurationTime] =
        useState(null);
    ...
    const classes = classNames([styles.validators, className]);

    useEffect(() => {
        if (validatorsLocations?.length > 0) {
            setCities(
                [...new Set(validatorsLocations.map((item) => item?.city))]
                    ?.length
            );
            setCountries(
                [...new Set(validatorsLocations.map((item) => item?.country))]
                    ?.length
            );
        }
    }, [validatorsLocations]);
    ...
    useEffect(() => {
        ...
        accounts
            .getAccountResourceData("0x1", "0x1::block::BlockResource")
            .then((conf: unknown) => {
                const configuration = conf as IApiAccountResourceDetails;
        ...
        accounts
            .getAccountResourceData("0x1", "0x1::staking_config::StakingConfig")
            .then((conf: unknown) => {
                const configuration = conf as IApiAccountResourceDetails;
                const { rewards_rate, rewards_rate_denominator } =
                    configuration?.data || {};
            ...
            <Grid>
                <GridWrapper gridWidth={mediaData.validatorsLeft}>
                    ...
                    <Grid>
                        <GridWrapper>
                            <Plate noMin min compressed>
                                <div className={"stats__item"}>
                                    {validatorsLocations?.length && (
                                        <div className={"stats__item-wrapper"}>
                                            <span className={"title"}>
                                                Nodes
                                            </span>
                                            <span className={"info break"}>
                                                {validatorsLocations?.length}
                                            </span>
                                        </div>
                                    )}
                        ...
                        <GridWrapper>
                            <Plate noMin min compressed>
                                {validatorsData?.timeRemaining !== null &&
                                validatorsData.percentage !== null ? (
                                    <div
                                        className={
                                            "stats__item-wrapper flex-start"
                                        }>
                                        <span className={"title"}>Epoch</span>
                                        <span className={"info break column"}>
                                            {concatString(
                                                validatorsData?.timeRemaining,
                                                "",
                                                " Minutes Remaining"
                                            )}
                                            <span className={"success"}>
                                                {concatString(
                                                    validatorsData?.percentage,
                                                    "",
                                                    "% complete"
                                                )}
                                            </span>
                                    ...
                                    <Skeleton style={{ minHeight: 40 }} />
                                )}
                                {total_voting_power !== null &&
                                validatorsData.rate !== null ? (
                                    <div
                                        className={
                                            "stats__item-wrapper flex-start"
                                        }>
