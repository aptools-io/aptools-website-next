...
const Stats: React.FC<{ time: string }> = ({ time = "0" }) => {
    ...
    const { slot = "0", slot_time_m = "0" } = aptosStats || {};
            ...
            {blockchain_info ? (
                <div className={styles["active-unique-addresses__stats"]}>
                    ...
                    <div className={styles["active-unique-addresses__stat"]}>
                        ...
                        <span
                            className={classNames([styles.info, styles.bold])}>
                            {slot_time_m}
                            <span>1min average</span>
                        </span>
                    </div>
                    <div className={styles["active-unique-addresses__stat"]}>
                        <span className={styles.title}>Slot time</span>
...
const ActiveUniqueAddresses: React.FC<IComponent> = ({ className }) => {
    const { data: generalData } = useSelector(
        (state: IRootState) => state.statsGeneral
    );
    ...
    const classes = classNames([styles["active-unique-addresses"], className]);
    ...
    return (
        <>
            <div className={classes}>
                <div
                    className={
                        styles["active-unique-addresses__title-wrapper"]
                    }>
                    <strong
                        className={styles["active-unique-addresses__title"]}>
                        Active Unique Addresses
                    </strong>
                    {active_unique_addresses ? (
                        <Grid
                            gap={6}
                            className={styles["active-unique-addresses__days"]}
                            columns={3}>
                            <GridWrapper
                                gridWidth={1}
                                className={
                                    styles["active-unique-addresses__day"]
                                }>
