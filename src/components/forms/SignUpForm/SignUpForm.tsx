...
const SignUpForm: React.FC = () => {
    ...
    const formik = useFormik(values(setLoading, dispatch));

    const classes = classNames([styles["sign-up-form"], "form"]);

    return (
        <FormikProvider value={formik}>
            <Form className={classes}>
                <strong className={"title"}>Sign up</strong>
                <div className={classNames([styles["sign-up-form__item"], "form__input", { error: formik.errors.email }])}>
                    <Field name={"email"} type='text'>
                        {({ field, form, meta }) => <TextInput label={"Account email"} placeholder={"email"} error={meta.touched && meta.error} field={field} />}
                    </Field>
                </div>
                <div className={classNames([styles["sign-up-form__item"], "form__input", { error: formik.errors.agreement }])}>
                ...
                <div className={classNames([styles["sign-up-form__item"], "form__input", { error: formik.errors.subscribe }])}>
                ...
                <div className={classNames([styles["sign-up-form__item-button"], "form__input"])}>
