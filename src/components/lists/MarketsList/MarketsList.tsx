...
const MarketsList: React.FC<IComponent> = ({ className }) => {
    ...
    const classes = classNames([styles.market, "list", className]);
    ...
    return (
        <div className={classes}>
            ...
            {markets?.length ? (
                <Tabs
                    tabsName={"marketsTabs"}
                    data={Object.fromEntries(
                        markets?.map((e) => [e.dex, e.pairs])
                    )}
                    itemsCount={false}>
                    <ListHeader columnNames={columnNames} columns={columns}>
                        <List adoptMobile />
                    </ListHeader>
                </Tabs>
            ) : (
