...
const Layout: React.FC<{ children: React.ReactNode; pageProps }> = ({
    children,
    pageProps
}) => {
    ...
    useEffect(() => {
        const getMetrics = (e) => {
            if (
                e.detail.name === "TTFB" ||
                e.detail.name === "Next.js-route-change-to-render"
            ) {
                notify(
                    dispatch,
                    `type: ${e.detail.name} <br/> loading time: ${(
                        e.detail.value / 1000
                    ).toFixed(3)}s`,
                    "Test"
                );
        ...
        <>
            <Head>
                <title>
                    {title
                        ? `${title} | Aptools`
                        : "Aptools – Making advanced on-chain analytics simple"}
                </title>
                ...
                <script
                    src={
                        "https://www.googletagmanager.com/gtag/js?id=G-J26EXPCWF1"
                    }
                    async
                />
                <script
            ...
            <noscript>
                <iframe
                    src='https://www.googletagmanager.com/gtag/js?id=G-J26EXPCWF1'
                    height='0'
                    width='0'
            ...
            <div
                ...
                <div
                    className={classNames([
                        styles["layout__page-wrapper"],
                        { [styles["space-between"]]: pageProps?.spaceBetween }
                    ])}>
                    {children}
                </div>
                <Footer />
