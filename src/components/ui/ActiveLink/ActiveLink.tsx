...
const ActiveLink = ({
    ...
    let className =
        `${child.props.className} ${styles["active-link__content"]}` || "";

    if (router.asPath === props.href) {
        if (onSamePathEmpty) return <></>;
        if (onSameAsPath) onSameAsPath(router.asPath === props.href);
        className = `${className} ${
            props.activeClassName ? props.activeClassName : ""
        }`.trim();
        return (
            <span onMouseEnter={props.onMouseEnter} className={className}>
    ...
    const handleClick = (e: React.MouseEvent<HTMLElement>) => {
        ...
        if (closestLink.classList.contains("prior-link"))
            link = closestLink.href;

        setRoute(router, 300, link || props.href, () =>
